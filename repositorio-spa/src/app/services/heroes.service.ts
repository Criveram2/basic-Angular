...
export class HeroesService {
  ...
  constructor() {
    console.log('servicio de heroes sin implementacion');
  }
  ...
  buscarHeroe(termino: string): Heroe[] {
    ...
    for (const heroeTemporal of this.heroes) {
      const nombre = heroeTemporal.nombre.toLowerCase();
      if (nombre.indexOf(termino) >= 0) {
        heroesArr.push(heroeTemporal);
      }
    }
    return heroesArr;
  }
